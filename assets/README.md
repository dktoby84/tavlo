...
|-----|----------|------------|
| `room.jpg` | `.hero-scene__photo`. Homografien i `js/main.js` er kalibreret mod denne fils naturlige mål (`PHOTO_W`/`PHOTO_H`) og tv-skærmens fire hjørner (`TV`). Whiteboardets fire hjørner er kalibreret i `.hero-scene__board`-markuppen (`index.html` / `en/index.html`) | Generisk hal, mørk/cinematisk, en tv-/skærmflade synligt på væggen der hvor `TV` peger, og en tom tavle på væggen. Ingen personer, logoer, brands eller tekst. Skiftes filen, skal BÅDE `TV` og tavlens hjørner måles om. |
